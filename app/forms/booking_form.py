...
def end_date_in_future(form, field):
    ...
    if end_date < datetime.today().strftime("%Y-%m-%d"):
        raise ValidationError('Checkout date cannot be in the past')     
...
class BookingForm(FlaskForm):
    ...
    start_date = StringField('Start_Date', validators=[DataRequired('Check-in date required'), start_date_in_future])
    end_date = StringField('End_Date', validators=[DataRequired('Checkout date required'), end_date_in_future])
